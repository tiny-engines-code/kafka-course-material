...
public class KafkaSpringApplication implements ApplicationRunner {
    ...
    public void examples(int recordCount) {
        Optional<String> optionalSchema = RegistryExample.doExample(topic,registry_url, NotificationStatus.class, "notification_status.avsc");
        if (optionalSchema.isPresent()) {
            GenericRecordFactory.setSchema(optionalSchema.get());
