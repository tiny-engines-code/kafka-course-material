...
public class NotificationStatusFactory  {
    ...
    public List<GenericRecord> next() {
        int terminal = 5;
    ...
    public GenericRecord nextStep(String upmid, int level) {
        ...
        ns.setReceived_time(Instant.ofEpochMilli(received_time.toEpochMilli()));
        ns.setStart_time(Instant.ofEpochMilli(start_time.toEpochMilli()));
        ns.setPublish_time(Instant.ofEpochMilli(publish_time.toEpochMilli()));
        ns.setEnd_time(Instant.ofEpochMilli(publish_time.toEpochMilli()));
