...
@Slf4j
public class ProducerRunnableJob<K, V> implements Runnable {

    private Thread worker;
    int records;
    ProducerWorker<K, V> producerWorker;
    UserFactory<V> userFactory;
    AtomicInteger sentCount = new AtomicInteger(0);

    public ProducerRunnableJob(String topicName, KafkaProducer<K, V> producer, UserFactory generator, int records) {
        Properties config = ProducerFactory.getProducerProperties();
        producerWorker = new ProducerWorker<K, V>(topicName, producer);
    ...
    public void run() {
        ...
        for (int i = 0; i < records; i++) {
            ...
            V nextValue = userFactory.nextUser();
    ...
    public void deadLetters(V record) {
